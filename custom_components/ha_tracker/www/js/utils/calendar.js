...
const FP_CDN = {
  coreJS:    'https://cdn.jsdelivr.net/npm/flatpickr@4.6.13/dist/flatpickr.min.js',
  coreCSS:   'https://cdn.jsdelivr.net/npm/flatpickr@4.6.13/dist/flatpickr.min.css',
  confirmJS: 'https://cdn.jsdelivr.net/npm/flatpickr@4.6.13/dist/plugins/confirmDate/confirmDate.js',
  confirmCSS:'https://cdn.jsdelivr.net/npm/flatpickr@4.6.13/dist/plugins/confirmDate/confirmDate.css',
  l10nBase:  'https://cdn.jsdelivr.net/npm/flatpickr@4.6.13/dist/l10n'
};
...
function detectFpLocale() {
  const supported = new Set(['ar','de','en','es','fr','hi','it','ja','pt','zh']);
...
async function ensureFlatpickrLoaded() {
  ...
  _fpReady = (async () => {
    ...
    await loadScriptOnce(FP_CDN.coreJS,    { test: () => !!window.flatpickr });
    await loadScriptOnce(FP_CDN.confirmJS, { test: () => !!window.confirmDatePlugin });

    // 3) Localización opcional
    const loc = detectFpLocale();
    if (loc !== 'en') {
      await loadScriptOnce(`${FP_CDN.l10nBase}/${loc}.js`, {
    ...
    console.log('[flatpickr] v', FP?.version, 'locale:', loc);
  })();
...
function hhmmFromHIS(his){ const [h="00",m="00"]=String(his||"").split(":"); return `${pad(h)}:${pad(m)}`; }

// ================================ Inicialización ================================
export async function initRangePicker(opts = {}) {
  _onApplyFilter = typeof opts.onApplyFilter === 'function' ? opts.onApplyFilter : null;
  if (window.__rangePickerInitDone) return;

  try {
    await ensureFlatpickrLoaded();
    injectFlatpickrShadowPatch();
  } catch (e) {
    console.error('[calendar] no se pudo cargar flatpickr:', e);
    return;
  }

  const daterangeEl = document.getElementById('daterange');
  if (!daterangeEl) { console.error('[calendar] #daterange no existe'); return; }
  ...
  fpRange = FP(daterangeEl, {
    ...
    allowInput: false,
    locale: localeObj,
    plugins: [ new Confirm({ confirmText: t('accept'), showAlways: true, theme: "light" }) ],
    ...
    onReady(_sel,_str,inst){ mountPanel(inst); writeLocalizedRange(inst); },
    onOpen(_sel,_str,inst){
      inst._prevTextboxValue = inst.input.value;
      inst.calendarContainer.classList.add('fp-at-top');
      mountPanel(inst);
      if (lastAppliedRange.length === 2) inst.setDate(lastAppliedRange, true);
      else writeLocalizedRange(inst);
    },
    onClose(_sel,_str,inst){ inst.calendarContainer.classList.remove('fp-at-top'); },
    onValueUpdate(){ writeLocalizedRange(fpRange); },
    onChange(){ writeLocalizedRange(fpRange); },
  });

  window.addEventListener('resize', () => {
    const cal = fpRange?.calendarContainer;
    if (cal) syncTimePickersWidth(cal);
  });
...
export function getSelectedLocalRange() {
  ...
  if (fpRange && Array.isArray(fpRange.selectedDates)) {
    ...
    if (len === 2) {
      ...
      const s = new Date(sDate); s.setHours(sh, sm, ss, 0);
      const e = new Date(eDate); e.setHours(eh, em, es, 0);
      startRaw = fmtLocal(s);
      endRaw   = fmtLocal(e);
    } else if (len === 1) {
      ...
      const [eh, em, es] = (endTimeCache   || "23:59:59").split(":").map(n => +n || 0);
      const s = new Date(sDate); s.setHours(sh, sm, ss, 0);
      const e = new Date(sDate); e.setHours(eh, em, es, 0);
      startRaw = fmtLocal(s);
      endRaw   = fmtLocal(e);
    }
  ...
  return { startLocal: startRaw, endLocal: endRaw };
}
...
export function setTimes(startHHMMSS = "00:00:00", endHHMMSS = "23:59:59") {
  ...
  fpStartTime?.setDate(startTimeCache, false);
  fpEndTime?.setDate(endTimeCache, false);
  if (fpRange) writeLocalizedRange(fpRange);
}
...
export function setRangeDates([startDate, endDate], updateInput = true) {
  ...
  if (!updateInput) writeLocalizedRange(fpRange);
}
...
export function updateDaterangeVisibility() {
  ...
  if (!personSelect || !daterangeBlock) return;
...
function writeLocalizedRange(instance) {
  if (!instance) return;
  const fmtDate = new Intl.DateTimeFormat(undefined, { year: "2-digit", month: "2-digit", day: "2-digit" });
...
function applySelection(instance) {
  ...
  instance.close();
  if (_onApplyFilter) _onApplyFilter();
}

function syncTimePickersWidth(mainCal) {
  const w = mainCal.getBoundingClientRect().width + "px";
  if (fpStartTime?.calendarContainer) fpStartTime.calendarContainer.style.width = w;
  if (fpEndTime?.calendarContainer)   fpEndTime.calendarContainer.style.width   = w;
}
...
function mountPanel(instance) {
  ...
  if (!panel) {
    ...
    const prefers24h = !new Intl.DateTimeFormat(undefined, { hour: "numeric" })
    ...
    fpStartTime = FP(startTimeAnchor, {
      ...
      locale: localeObj,
      onChange: (_, dateStr) => { startTimeCache = dateStr || "00:00:00"; }
    });
    fpEndTime = FP(endTimeAnchor, {
      ...
      locale: localeObj,
      onChange: (_, dateStr) => { endTimeCache = dateStr || "23:59:59"; }
    });
...
function firstOfMonth(d){ const x=new Date(d.getFullYear(), d.getMonth(), 1); x.setHours(0,0,0,0); return x; }
function firstOfNextMonth(d){ const x=new Date(d.getFullYear(), d.getMonth()+1, 1); x.setHours(0,0,0,0); return x; }
...
function startOfNextISOWeek(d){ const start=startOfISOWeek(d); const next=new Date(start); next.setDate(start.getDate()+7); return new Date(next.getTime()-1000); }
...
function getQuickRange(value) {
  ...
  switch (value) {
    case 'today':       return { start: today0, end: today0 };
    case 'yesterday':   return { start: addDays(today0,-1), end: addDays(today0,-1) };
    case 'last_24h':    return { start: new Date(now - 24*60*60*1000), end: new Date(now - 1000) };
    case 'last_7_days': return { start: new Date(now - 7*24*60*60*1000), end: now };
    case 'this_month':  return { start: firstOfMonth(now), end: firstOfNextMonth(now) };
    case 'last_month': {
    ...
    case 'this_week':   return { start: startOfISOWeek(now), end: startOfNextISOWeek(now) };
...
function injectFlatpickrShadowPatch() {
  if (document.getElementById('fp-shadow-patch')) return;
